...
def PltTime(Time,Data,Legend,Label,XRange=0,YRange=0,XTicks=0,YTicks=0,Save=''):
    fig = plt.figure(figsize=(8,6))
    ax = fig.gca()
    ax.xaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))
    ax.xaxis.set_major_locator(matplotlib.dates.MinuteLocator(interval=XTicks[1]))
    ax.yaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))
    ax.yaxis.set_major_locator(matplotlib.ticker.MultipleLocator(YTicks[1]))

    ax.grid(b=True, which='major', color='k', linestyle='-')
    ax.grid(b=True, which='minor', color='k', linestyle=':', alpha=0.8)
    ...
    for ii, yy in enumerate(Data):
        plt.plot(Time, yy, 'o', label=Legend[ii], color=colors[ii], marker='o', mew=0.1)
    ax.legend(loc='lower left',bbox_to_anchor=(0.005,0.92), ncol=3, borderaxespad=0, fontsize=14)
    fig.tight_layout()
    if Save:
...
def PltWfm(Time,Data,Legend,Label=['Time [$\mu$s]', 'Amplitude [mV]'],XRange=0,YRange=0,XTicks=0,YTicks=0,Color='',Save=''):
    fig = plt.figure(figsize=(10,6))
    ax = fig.gca()
    ax.grid(b=True, which='major', color='k', linestyle='-')
    ax.grid(b=True, which='minor', color='grey', linestyle=':')
    ax.xaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))
    ax.xaxis.set_major_locator(matplotlib.ticker.MultipleLocator(XTicks[1]))
    ax.yaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))
    ax.yaxis.set_major_locator(matplotlib.ticker.MultipleLocator(YTicks[1]))
    plt.xlabel(Label[0])
    plt.ylabel(Label[1])
